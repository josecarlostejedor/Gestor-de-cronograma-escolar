...
# Si rellenas estas dos variables, la app se conectará automáticamente
# y no pedirá las claves en la barra lateral.
FIXED_API_KEY = "$2a$10$RPxoHbj3Z7oFeHcVX9KXNeQSlPDUcN/a0C1n7bFO6RcsNLROAwRaq"  # Pega aquí tu API Key
FIXED_BIN_ID = "6986233f43b1c97be96a93ee"   # Pega aquí tu Bin ID
...
if FIXED_API_KEY and FIXED_BIN_ID:
    use_cloud = True
    api_key = FIXED_API_KEY
    bin_id = FIXED_BIN_ID
    st.sidebar.success("☁️ Nube Activada (Credenciales Internas)")
else:
    st.sidebar.title("☁️ Configuración")
    st.sidebar.markdown("Para sincronizar dispositivos, introduce tus claves de JSONBin.io:")
    use_cloud_check = st.sidebar.checkbox("Activar Sincronización", value=False)
    if use_cloud_check:
        api_key_input = st.sidebar.text_input("API Key (Master Key)", type="password")
        bin_id_input = st.sidebar.text_input("Bin ID")
        if api_key_input and bin_id_input:
            use_cloud = True
            api_key = api_key_input
            bin_id = bin_id_input
            st.sidebar.success("Conectado")
        else:
            st.sidebar.warning("Faltan datos")
    else:
        st.sidebar.info("Modo Local (Solo este dispositivo)")
...
<html lang="es">
  ...
  <body class="bg-gray-100 text-gray-900">
    ...
    <script type="text/babel" data-type="module">
      ...
      import { Plus, Calendar as CalendarIcon, MapPin, Users, Building2, X, Download, Lock, Trash2, AlertTriangle, Save, Cloud, CloudOff, Loader2, GraduationCap } from 'lucide-react';
      ...
      const generateWordDocument = async (activities) => {
          ...
          const dayCells = DAYS.map((_, dayIndex) => {
            ...
            const paragraphs = cellActivities.length > 0 
              ? cellActivities.flatMap(act => [
                  ...
                  new Paragraph({ text: `  Dept. Org: ${act.department}`, size: 18 }),
                  new Paragraph({ text: `  Prof. Resp: ${act.teacher || 'N/A'}`, size: 18 }),
        ...
        const doc = new Document({
          sections: [{
            ...
            children: [
              new Paragraph({ text: "Semana Cultural IES Lucía de Medrano (App made by JC)", heading: HeadingLevel.HEADING_1, alignment: AlignmentType.CENTER, spacing: { after: 400 } }),
      ...
      const ActivityModal = ({ isOpen, onClose, onSave, initialDayIndex, initialSlotIndex }) => {
        ...
        const handleSubmit = (e) => {
          e.preventDefault();
          onSave({ title, department, teacher, group, location, dayIndex: initialDayIndex, slotIndex: initialSlotIndex });
          setTitle(''); setDepartment(''); setTeacher(''); setGroup(''); setLocation(''); onClose();
        };
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-md overflow-hidden animate-in fade-in zoom-in duration-200">
              ...
              <form onSubmit={handleSubmit} className="p-6 space-y-4">
                ...
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Departamento Organizador</label><input required type="text" value={department} onChange={(e) => setDepartment(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all" placeholder="Ej: Matemáticas" /></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">Profesor Responsable</label><input required type="text" value={teacher} onChange={(e) => setTeacher(e.target.value)} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 outline-none transition-all" placeholder="Ej: Juan Pérez" /></div>
      ...
      const ExportButton = ({ activities }) => {
        ...
        return (
          <>
            <button onClick={() => { setIsModalOpen(true); setPassword(''); setError(''); }} className="flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-6 py-3 rounded-lg font-bold shadow-lg transition-all transform hover:scale-105 hover:shadow-green-200">
      ...
      const ResetButton = ({ onReset }) => {
        ...
        return (
          <>
            <button onClick={() => { setIsModalOpen(true); setPassword(''); setError(''); }} className="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-4 py-3 rounded-lg font-bold shadow-lg transition-all transform hover:scale-105 hover:shadow-red-200" title="Borrar todas las actividades">
      ...
      const App = () => {
        ...
        return (
          <div className="min-h-screen bg-gray-100 flex flex-col">
            ...
            <header className="bg-white/80 backdrop-blur-md shadow-sm border-b border-gray-200 sticky top-0 z-40 transition-all duration-300">
              <div className="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 h-20 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="bg-indigo-600 p-2 rounded-lg text-white shadow-indigo-200 shadow-lg"><CalendarIcon size={28} /></div>
                  <div>
                    <h1 className="text-xl sm:text-2xl font-bold text-gray-900 leading-none tracking-tight flex flex-col sm:flex-row sm:items-end gap-1 sm:gap-2">
                      Semana Cultural IES Lucía de Medrano
                      <span className="text-xs font-normal text-gray-500 pb-0.5">(App made by JC)</span>
                    </h1>
                    <div className="flex items-center gap-2 mt-1">
                      <p className="text-sm text-gray-500 font-medium">23 - 27 de Marzo</p>
                      {CLOUD_CONFIG.enabled ? (
                        <span className="text-xs bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full flex items-center gap-1 border border-blue-200">
                          {isSaving ? <Loader2 size={10} className="animate-spin" /> : <Cloud size={10} />}
                          {isSaving ? 'Guardando...' : 'Sincronizado'}
                        </span>
                      ) : (
                        <span className="text-xs bg-gray-200 text-gray-600 px-2 py-0.5 rounded-full flex items-center gap-1 border border-gray-300">
              ...
              <div className="max-w-8xl mx-auto bg-white rounded-xl shadow-xl overflow-hidden border border-gray-200">
                <div className="overflow-x-auto">
                  <div className="min-w-[1200px]">
                    ...
                    {/* FIX: Changed top-20 to top-0 so it sticks to the top of the scroll container correctly */}
                    <div className="sticky top-0 z-30 grid grid-cols-[120px_repeat(5,1fr)] border-b-2 border-gray-200 bg-gray-50 shadow-sm">
                      <div className="p-4 font-bold text-gray-400 text-center flex items-center justify-center border-r border-gray-200 bg-gray-50">HORA</div>
                      {DAYS.map((day, index) => (
                        <div key={index} className="p-4 font-bold text-indigo-900 text-center text-lg border-r border-gray-200 last:border-r-0 bg-gray-50">{day}</div>
                      ))}
                    </div>
                      ...
                      if (isRecess) {
                        return (
                          <div key={slot.id} className="grid grid-cols-[120px_1fr] bg-amber-50 border-b border-gray-200">
                            ...
                            <div className="p-4 flex items-center justify-center text-amber-700 font-bold tracking-widest text-xl bg-amber-50/50">RECREO</div>
                          </div>
                        );
                      }
                              ...
                              <div key={dayIndex} className="relative p-2 border-r border-gray-200 last:border-r-0 group hover:bg-gray-50 transition-colors">
                                <div className="space-y-3 mb-8">
                                  {cellActivities.map((act) => (
                                    <div key={act.id} className="bg-white border-l-4 border-indigo-500 shadow-sm rounded-lg p-3 text-sm hover:shadow-md hover:-translate-y-1 transition-all duration-200 border border-gray-100 group/card">
                                      <div className="font-bold text-gray-900 mb-1.5 text-base">{act.title}</div>
                                      <div className="space-y-1 text-xs text-gray-600">
                                        <div className="flex items-center gap-1.5"><Building2 size={14} className="text-indigo-500" /><span>{act.department}</span></div>
                                        {act.teacher && (
                                          <div className="flex items-center gap-1.5"><GraduationCap size={14} className="text-indigo-500" /><span>{act.teacher}</span></div>
                                        )}
                                        <div className="flex items-center gap-1.5"><Users size={14} className="text-indigo-500" /><span>{act.group}</span></div>
                                        <div className="flex items-center gap-1.5"><MapPin size={14} className="text-indigo-500" /><span>{act.location}</span></div>
                                      </div>
                                    </div>
                                  ))}
                                </div>
                                <button onClick={() => handleAddClick(dayIndex, slot.id)} className={`absolute bottom-2 right-2 p-2 rounded-full bg-indigo-50 text-indigo-600 hover:bg-indigo-600 hover:text-white transition-all shadow-sm hover:shadow-md ${cellActivities.length === 0 ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`} title="Añadir actividad">
                                  <Plus size={20} />
                                </button>
                              </div>
                            );
