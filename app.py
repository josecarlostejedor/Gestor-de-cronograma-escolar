...
FIXED_API_KEY = ""  # Ejemplo: "$2a$10$RPxoHbj3Z7oFeHcVX9KXNeQSlPDUcN/a0C1n7bFO6RcsNLROAwRaq"
FIXED_BIN_ID = ""   # Ejemplo: "6986233f43b1c97be96a93ee"

# Configuración de la página de Streamlit
st.set_page_config(
    page_title="Calendario Escolar Conectado",
    layout="wide",
    initial_sidebar_state="collapsed" # Colapsado por defecto si ya está configurado
)
...
# Comprobamos si hay credenciales fijas
if FIXED_API_KEY and FIXED_BIN_ID:
    use_cloud = True
    api_key = FIXED_API_KEY
    bin_id = FIXED_BIN_ID
    # Indicador discreto en la barra lateral
    st.sidebar.success("☁️ Nube Activada (Credenciales Internas)")
else:
    # Si no hay credenciales fijas, mostramos el formulario
        ...
        async load() {
          if (CLOUD_CONFIG.enabled) {
            try {
              ...
              // Si falla la carga silenciosa, no alertamos constantemente, solo retornamos vacío o local
              return [];
            }
          } else {
